...
![Three-panel schematic showing source-region material on the left (mineral dust, crop residue, fungal spores, organochlorine residues, SO2, NO2, ammonia), chemical transformation in the center (ammonium sulfate formation, secondary organics, organochlorine photolysis, spores adsorbing onto dust, protein fragmentation), and the arrived material on the right (aged sulfate and organic aerosol, modified spore surfaces, dust-bound protein complexes, reaction products absent at the source). Connected by a five-day transit arrow and a deposition arrow, with a sampling note at the bottom: source-region grab samples miss the reaction products that form in flight.](/assets/img/posts/kawasaki-wind-patterns/chemistry-transit.png)

*What leaves the source and what arrives at the receptor are chemically different. This is why source-region grab samples can systematically miss the actual exposure, and why flight sampling over Japan — however imperfect — captures something no ground-based sampling of the upstream region can.*
...
![Animated timeline 1950-1967 of the upstream source zone: the map starts empty, then the Northeast China industrial belt fills in across Shenyang, Anshan, Fushun, Changchun, and Harbin (1953-57); the Virgin Lands Campaign hatching spreads across Kazakhstan steppe (1953-65); the Great Leap Forward deforestation hatching appears across Manchuria (1957-64); and finally a red star marks Japan's first Kawasaki disease case in 1961. A progress bar along the bottom tracks the three campaigns against the current year.](/assets/img/posts/kawasaki-wind-patterns/emergence-timeline.gif)

*Three overlapping land-surface disruptions across the source swath, 1950 to 1967. The industrial belt arrives first, then the Virgin Lands Campaign begins lofting Kazakh steppe soil westward of the swath, then the Great Leap Forward strips Manchurian forest cover on its eastern edge. Japan's first recognized Kawasaki case appears in 1961 — within the decade when the upstream land surface was being transformed faster than at any point in the region's recorded history.*
...
The wind analysis figures were generated from NOAA PSL NCEP/NCAR Reanalysis 1 monthly wind fields for 1996–2006 using [`scripts/generate_kawasaki_wind_article_images.py`](https://github.com/ndjstn/ndjstn.github.io/blob/main/scripts/generate_kawasaki_wind_article_images.py). The back-trajectory and AERONET figures were generated by [`scripts/kawasaki_open_data_analysis.py`](https://github.com/ndjstn/ndjstn.github.io/blob/main/scripts/kawasaki_open_data_analysis.py), which fetches an 850 hPa wind domain extending west to 65°E for the trajectory integration and downloads AERONET Level 2.0 daily AOD via the public API. Both scripts access NOAA data through OPeNDAP and cache regional subsets under `.cache/kawasaki-wind/`. Package versions are in [`requirements-kawasaki-wind.txt`](https://github.com/ndjstn/ndjstn.github.io/blob/main/requirements-kawasaki-wind.txt).
