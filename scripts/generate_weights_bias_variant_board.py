...
def variant_a(ax):
    ...
    for (_, y, _, _), label in zip(inputs, weight_labels, strict=False):
        ax.text(
            0.30,
            y + 0.045,
            label,
    ...
    ax.text(
        0.50, 0.535, "score", ha="center", va="center", fontsize=15, fontweight="bold"
    )
    ax.text(
        0.50,
        0.48,
    ...
    ax.text(
        0.58, 0.75, "bias shifts the whole score", fontsize=9.5, color=COLORS["muted"]
    )
...
def variant_b(ax):
    ...
    for feature, value, color, y in zip(
        features, values, colors, [0.75, 0.64, 0.53, 0.42, 0.31], strict=False
    ):
    ...
    ax.text(
        0.08, 0.22, rf"total score $z$ = {total:.2f}", fontsize=11.5, fontweight="bold"
    )
    ax.text(
        0.08,
        0.17,
        rf"activation $\sigma(z)$ = {prob:.2f}",
        fontsize=10.7,
        color=COLORS["muted"],
    )
    ...
    ax.text(
        0.74,
        0.39,
        "threshold shifts are visible here too",
        fontsize=9.5,
        color=COLORS["muted"],
    )
...
def save_board():
    ...
    fig.text(
        0.05,
        0.955,
        "Weights and bias: three ways to teach the same idea",
        fontsize=24,
        fontweight="bold",
        color=COLORS["ink"],
    )
    fig.text(
        0.05,
        0.918,
        "Pick the version that makes the interaction clearest. We can also combine the best parts into a hybrid.",
        fontsize=12.2,
        color=COLORS["muted"],
    )
    gs = fig.add_gridspec(
        1, 3, left=0.04, right=0.98, top=0.87, bottom=0.07, wspace=0.08
    )
